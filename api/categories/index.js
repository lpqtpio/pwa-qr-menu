    ...
    const enhancedCategories = categories.map(category => {
      ...
      return {
        ...
        stats: {
          ...
          priceRange: {
            min: categoryDishes.length ? Math.min(...categoryDishes.map(d => d.price)) : 0,
            max: categoryDishes.length ? Math.max(...categoryDishes.map(d => d.price)) : 0
          }
        },
  ...
  try {
    ...
    if (!menu) {
      ...
      if (!newCategory.order) {
        const maxOrder = Math.max(...menu.categories.map(c => c.order || 0), 0);
